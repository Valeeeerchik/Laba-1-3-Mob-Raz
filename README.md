...
1. Git clone ````https://github.com/Valeeeerchik/Laba-1-3-Mob-Raz.git````
...
Запустив приложение вам будет доступны кнопки button1, поделиться, Make Call нажмите её чтобы передать параметр заданный внутри кнопки. 
Нажав на кнопку button1, откроется камера, чтобы сделать фото
Нажав на кнопку поделиться, откроется окно с иконками мессенджеров (telegram, WhatsApp, VK, и.т.д.) чтобы поделиться сделанной фотографией 
Нажав на кнопку Make Call, откроется прижение для набора номера. И там будет представлен номер поддержки.
